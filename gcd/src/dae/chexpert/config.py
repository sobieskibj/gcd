...
class TrainConfig(BaseConfig):
    ...
    def make_dataset(self, path=None, **kwargs):
        if self.data_name == 'ffhqlmdb256':
            ...
            return ImageNetDataset(image_size = self.img_size, **kwargs)
        elif self.data_name == 'chexpert':
            return CheXpertDataset(image_size = self.img_size, **kwargs)
        else:
            raise NotImplementedError()
    ...
    def make_model_conf(self):
        if self.in_channels is not None:
            ...
            print('in_channels not found in self, setting in_channels to 3')
            in_channels = 3
