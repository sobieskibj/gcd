...
class CelebAlmdb(Dataset):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        self.original_resolution = original_resolution
        self.data = BaseLMDB(path, original_resolution, zfill = zfill)
        self.length = len(self.data)
...
class ImageNetDataset(Dataset):
    ...
    def __init__(
            self, 
            path_data = 'evanarlian/imagenet_1k_resized_256', 
            split = 'train', 
            image_size = 256,
            ...
            as_tensor = True):
        super().__init__()
        self.data = load_dataset(path_data)[split]
        self.split = split
        self.length = self.data.num_rows

        transform = [transforms.CenterCrop(center_crop_size), transforms.Resize(image_size)]
        if do_augment:
            transform.append(transforms.RandomHorizontalFlip())
        if as_tensor:
            transform.append(transforms.ToTensor())
    ...
    def __getitem__(self, index):
        img_label = self.data[index]
        img = self.transform(img_label['image'])
...
class CheXpertDataset(Dataset):
    ...
    def __init__(
        ...
        self.image_size = 224
        self.n_channels = 1
        assert path_data is not None, 'Provide path to dataset'
        assert path_csv is not None, 'Provide path to .csv with metadata'
        if n_channels == 3:
            self.image_read_mode = ImageReadMode.RGB
        elif n_channels == 1:
            self.image_read_mode = ImageReadMode.UNCHANGED
        else:
            raise ValueError(f'n_channels should be either 1 or 3, not {n_channels}')
    ...
    def read_csv(self):
        ...
        paths = df.loc[~df.index.str.contains('lateral')].index.values
        self.paths = ['/'.join(p.split('/')[1:]) for p in paths]
    ...
    def __getitem__(self, index):
        img_path = self.path_data / self.paths[index]
        img = read_image(str(img_path), mode = self.image_read_mode) / 255
        img = self.transform(img)
        return {'img': img, 'index': index, 'labels': 0}
