...
class LitModel(pl.LightningModule):
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx: int,
        ...
        # dataloader_idx: int excluded for debug
        """
    ...
    def on_before_optimizer_step(
        ...
        # optimizer_idx: int excluded for debug
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    trainer = pl.Trainer(
        ...
        # resume_from_checkpoint=resume, # Deprecated in new pl version
        devices=gpus, # gpus -> devices in new pl version
        num_nodes=nodes,
        # accelerator=accelerator, # NOTE: Commented for debug
        ...
        # replace_sampler_ddp=True, # Deprecated in new pl version
        logger=tb_logger,
        accumulate_grad_batches=conf.accum_batches,
        # plugins=plugins, # NOTE: Commented for debug
    )

    if mode == 'train':
        ckpt_path = resume if conf.use_ckpt else None
